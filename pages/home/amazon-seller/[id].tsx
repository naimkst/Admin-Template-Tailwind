...
export default function editAmazonSeller(): any {
  ...
  const { isLoading, data, isSuccess, mutate, mutateAsync } =
    homeAmazonSellerUpdate(currentRoute);
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    ...
    formData.append("amazonTitle", data.amazonTitle);
    formData.append("amazonDescription", data.amazonDescription);
    formData.append("image", formImage);
    formData.append("amazonUrl", data.amazonUrl);
    ...
    setWaiting(false);
    setButtonHide(true);
    router.push("/home/amazon-seller");
  };

  const { isLoading: loading }: any = useQuery(["resTestimonail"], async () => {
    const { data } = await request.get(`/home/amazon-seller/${currentRoute}`);
    reset(data);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div>
            ...
            <input
              ...
              required
              defaultValue=""
              {...register("amazonTitle", {
                required: "Title is required",
              })}
            />
            {errors.amazonTitle && (
              <p className="mt-2 text-sm text-red-500">
                {errors.amazonTitle?.message}
              </p>
            )}
          </div>
          ...
          <div className="mb-6 mt-6">
            ...
            <textarea
              ...
              required
              defaultValue=""
              {...register("amazonDescription", {
                required: "Content is required",
              })}
            />
            {errors.amazonDescription && (
              <p className="mt-2 text-sm text-red-500">
                {errors.amazonDescription?.message}
              </p>
            )}
          </div>
