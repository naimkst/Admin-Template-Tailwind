...
import { ecommerceUpdate } from "../../../service/service";
...
export default function editMinds(): any {
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    ...
    setWaiting(false);
    setButtonHide(true);
    router.push("/service/ecommerce");
  };

  const { isLoading: loading }: any = useQuery(["resTestimonail"], async () => {
    const { data } = await request.get(`/service/ecommerce/${currentRoute}`);
    reset(data);
  ...
  return (
    <>
      ...
      <div className="max-w-6xl bg-white p-16">
        <h2 className="text-2xl mb-10 font-bold">Edit</h2>
